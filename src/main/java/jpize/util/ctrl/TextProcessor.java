...
import jpize.util.font.FontCharset;
...
public class TextProcessor implements Disposable, GlfwCharCallback, GlfwKeyCallback {
    ...
    public TextProcessor(boolean enabled, boolean newLineOnEnter) {
        ...
        Jpize.callbacks().addCharCallback(this);
        Jpize.callbacks().addKeyCallback(this);
    }
    ...
    @Override
    public void invoke(GlfwWindow window, char character) {
        if(!enabled)
            return;
    ...
    @Override
    public void invoke(GlfwWindow window, Key key, int scancode, GlfwAction action, GlfwMods mods) {
        if(!enabled || action == GlfwAction.RELEASE)
            return;
    ...
    public void insertText(String text) {
        int linesNum = StringUtils.countMatches(text, "\n") + 1;
    ...
    public void dispose() {
        lines.clear();
        Jpize.callbacks().removeCharCallback(this);
        Jpize.callbacks().removeKeyCallback(this);
    }
